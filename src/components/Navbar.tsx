      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center h-16 gap-4">
          ...
          >
            ...
            <span className="font-bold text-slate-900 text-lg hidden sm:block">PiaтaRO</span>
          </button>
          ...
          <div className="flex-1 max-w-2xl relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 bg-slate-100 border border-transparent rounded-xl text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:bg-white focus:border-blue-400 transition-all"
            />
          </div>
            ...
            {user ? (
              <>
                ...
                <button
                  ...
                  className={`p-2 rounded-xl transition-colors ${currentPage === "favorites" ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-slate-100"}`}
                >
                  <Heart className="w-5 h-5" />
                </button>
                <button
                  onClick={() => onNavigate("messages")}
                  className={`p-2 rounded-xl transition-colors ${currentPage === "messages" ? "bg-blue-50 text-blue-600" : "text-slate-600 hover:bg-slate-100"}`}
                >
                ...
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 p-2 rounded-xl text-slate-600 hover:bg-slate-100 transition-colors"
                  >
              ...
              <>
                <button
                  ...
                  className="text-sm text-slate-600 font-medium hover:text-slate-900 px-3 py-2 rounded-xl hover:bg-slate-100 transition-colors"
                >
                  Autentificare
                </button>
                <button
        ...
        <div className="md:hidden border-t border-slate-200 bg-white px-4 py-4 space-y-2">
          {user ? (
            <>
              ...
              <button onClick={() => { onNavigate("profile"); setMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 rounded-xl flex items-center gap-2">
                <User className="w-4 h-4" /> Profilul meu
              </button>
              <button onClick={() => { onNavigate("my-listings"); setMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 rounded-xl flex items-center gap-2">
                <ShoppingBag className="w-4 h-4" /> Anunturile mele
              </button>
              <button onClick={() => { onNavigate("favorites"); setMenuOpen(false); }} className="w-full text-left px-4 py-3 text-sm text-slate-700 hover:bg-slate-50 rounded-xl flex items-center gap-2">
